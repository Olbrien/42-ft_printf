/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   ifcharacter.c                                      :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: tisantos <tisantos@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/01/16 07:13:32 by tisantos          #+#    #+#             */
/*   Updated: 2021/01/17 06:26:18 by tisantos         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../includes/ft_printf.h"

int		character_write_minus_greater(p_list *plist, s_list *slist, int i)
{
	while (i < slist->minus - 1)
	{
		write(1, " ", 1);
		plist->final_format_lenght++;
		i++;
	}
	slist->minus = 0;
	return(i);
}

int		character_write_width_greater(p_list *plist, s_list *slist, int i)
{
	while (i < slist->width - 1)
	{
		write(1, " ", 1);
		plist->final_format_lenght++;
		i++;
	}
	slist->width = 0;
	return(i);
}

void	character_process(p_list *plist, char character, s_list *slist)
{
	int i;

	i = 0;
	if (!plist) // || !character) <-- Tirar isto é necessário para casos \0 (duvidoso)
		return ;
	if (slist->width > 0 && slist->minus == 0)
		i = character_write_width_greater(plist, slist, i);
	write(1, &character, 1);
	if (slist->minus > 0)
		i = character_write_minus_greater(plist, slist, i);
	if (plist->final_format == NULL)
	{
		if (!(plist->final_format = malloc(sizeof(char) + 2)))
			return ;
		plist->final_format[0] = character;
		plist->final_format[1] = '\0';
	}
	else
		plist->final_format = ft_strjoinchr(plist->final_format, character);
	plist->final_format_lenght++;
}


void	ifcharacter(p_list *plist, s_list *slist, va_list *args)
{
	unsigned char send;

	send = (unsigned char)va_arg(*args, int);

	character_process(plist, send, slist);
	plist->format_count++;
}
