...
/*   Updated: 2021/01/20 04:47:24 by tisantos         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	write_character_outside_conversion(p_list *plist, char character, s_list *slist)
{
	...
	i = 0;
	if (!plist) // || !character) <-- Tirar isto é necessário para casos \0 (duvidoso)
		return ;
	if (slist->width > 0 && slist->minus == 0)
		i = character_outside_conversion_write_width_greater(plist, slist, i);
	write(1, &character, 1);
	if (slist->minus > 0)
		i = character_outside_conversion_write_minus_greater(plist, slist, i);
...
int		ft_printf(const char *format, ...)
{
	...
	while (plist.format_count < plist.format_length)
	{
		...
		{
			write_character_outside_conversion(&plist, plist.format[plist.format_count], &slist);
			plist.format_count++;
			initialize_slist(&slist);
		}
	}
...
# define cconversions "cspdiuxX%nfge"
# define cspecifiers  "-0.*lh# +" // width também e l ll h hh

// TODO
// Estavas a acabar o precision.
...
// Nos testes das strings falta testar as flags "+-0 "
// Nos testes das %% falta testar os "0"
